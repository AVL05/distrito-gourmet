...
const OrderItem = ({ order }) => {
  ...
  const pickupTime = order.pickup_time ? new Date(order.pickup_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : null;
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: return 'text-text-muted bg-text-muted/10 border-text-main/10';
    }
  };
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      ...
      case 'ready': return 'Listo para Recogida';
      case 'delivered': return 'Recogido';
  ...
  return (
    <motion.div
      layout
      className="group relative bg-bg-surface/50 border border-text-main/10 p-6 sm:p-8 hover:bg-text-main/[0.07] transition-all duration-300 rounded-sm">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-start gap-4 flex-1">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-3 mb-2">
              ...
              <span className={`text-[9px] uppercase tracking-widest px-2.5 py-0.5 border rounded-full font-bold ${getStatusColor(order.status)}`}>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
           ...
           >
             {showDetails ? 'Ocultar Detalles' : 'Ver Qué se Pedido'}
           </button>
        </div>
      </div>
            ...
            <div className="mt-8 pt-8 border-t border-text-main/10 space-y-6">
              ...
              <div className="space-y-3">
                {order.items?.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center text-sm font-body py-3 border-b border-text-main/5 last:border-0 hover:bg-text-main/[0.02] px-2 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-text-main font-medium tracking-tight uppercase text-[13px]">{item.item_name}</span>
                        <span className="text-[10px] text-text-muted tracking-widest">{parseFloat(item.price).toFixed(2)}€ / Ud.</span>
                      </div>
                    </div>
                    <span className="text-text-main font-heading text-base font-medium">{(item.quantity * item.price).toFixed(2)}€</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-between items-center pt-6 px-2 border-t border-text-main/10 text-text-main mt-4">
                 <div className="flex flex-col">
                   <span className="text-[9px] uppercase tracking-[3px] text-text-muted mb-1 font-bold">Resumen de Pago</span>
                   <span className="text-[12px] font-medium italic opacity-70">Abonado mediante: {order.payment_method === 'card' ? 'Tarjeta Bancaria' : order.payment_method === 'paypal' ? 'PayPal' : 'Efectivo en Local'}</span>
                 </div>
                 <div className="text-right">
                   <div className="text-[10px] uppercase tracking-[3px] text-text-muted mb-1 font-bold">Suma Total</div>
                   <div className="text-2xl font-heading text-primary leading-none">{parseFloat(order.total).toFixed(2)}€</div>
                 </div>
              </div>
            </div>
          </motion.div>
